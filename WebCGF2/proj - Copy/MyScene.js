...
class MyScene extends CGFscene {
    ...
    init(application) {
        ...
        this.autoPilotEnableRate = 500;
        this.enableThirdPersonRate = 501;
    ...
    checkKeys() {
        ...
        if (this.gui.isKeyPressed("KeyP")) {
            if(this.autoPilotEnableRate>500){
                if(this.myblimp.autoPilot == false){
                    this.myblimp.setAutoPilot(true);
                    this.myblimp.calculateAutoPilotCenter();
                    this.myblimp.turning = 1;
                }else {
                    this.myblimp.setAutoPilot(false);
                    this.myblimp.calculateAutoPilotCenter();
                    this.myblimp.turning = 0;
                }
                this.autoPilotEnableRate = 0;
        ...
        if (this.gui.isKeyPressed("KeyY")) {
            if(this.enableThirdPersonRate>500){
                if(this.isFirstPersonActive == false){
                    this.isFirstPersonActive = true;
                }else{
                    this.isFirstPersonActive = false;
                }
                
                this.enableThirdPersonRate = 0;
            }
        }
    ...
    update(t){
        ...
        this.autoPilotEnableRate+=this.period;
        
        this.enableThirdPersonRate += this.period;

        if(this.myblimp.isAutoPilot()){
            this.myblimp.updateAutoPilot(this.period);
    ...
    updateFirstPersonCamera() {
        this.camera.setPosition([this.myblimp.getPosition()[0] + Math.cos(this.myblimp.getOrientationAngle())-5, this.myblimp.getPosition()[1]+5, this.myblimp.getPosition()[2] + Math.sin(this.myblimp.getOrientationAngle())-5])
        this.camera.setTarget([this.myblimp.getPosition()[0] + Math.cos(this.myblimp.getOrientationAngle()), this.myblimp.getPosition()[1], this.myblimp.getPosition()[2] + Math.sin(this.myblimp.getOrientationAngle())])
    }
