class MyBlimp extends CGFobject {
    ...
    constructor(scene, slices, stacks) {
        ...
        this.flagShader_invert.setUniformsValues({uSampler1: 1});
    ...
    initMaterials(){
        ...
        this.flagAppearance.setTexture(this.flagTexture2);
    ...
    calculateAutoAngle(period) {
        this.autoAngle = (period/1000)*2*Math.PI/5;
    }

    updateAutoPilot(period) {
        this.velocity = Math.sqrt(Math.pow((period/1000)*2*Math.PI/5,2)*25);
        this.autoAngle = (period/1000)*2*Math.PI/5;
        this.orientationAngle+=this.autoAngle;
    }

    calculateAutoPilotCenter() {
        this.autoPilotCenter = [this.position[0]+5*Math.cos(this.orientationAngle + Math.PI/2), 0, this.position[2]+5*Math.sin(this.orientationAngle + Math.PI/2)];
    }
